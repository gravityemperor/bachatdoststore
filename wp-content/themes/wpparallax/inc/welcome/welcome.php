<?php
	if(!class_exists('Wpparallax_Welcome')) :

		class Wpparallax_Welcome {
			...
			public function __construct( $plugins, $strings ) {
				...
				$this->tab_sections = array(
					'getting_started' 		=> esc_html__('Getting Started', 'wpparallax'),
					'recommended_plugins' 	=> esc_html__('Recommended Plugins', 'wpparallax'),
					'demo_import' 			=> esc_html__('Import Demo', 'wpparallax'),
					'changelogs'			=> esc_html__('Changelogs','wpparallax'),
					'free_vs_pro'			=> esc_html__('Free Vs Pro','wpparallax'),
			...
			<p>
			<?php
			printf( '%1$s %2$s %3$s <a href="%4$s">%5$s</a> %6$s', esc_html__( 'Welcome! Thank you for choosing', 'wpparallax' ), esc_html($this->theme_name), esc_html__( 'Please make sure you visit our', 'wpparallax' ), esc_url( admin_url( 'themes.php?page=welcome-page' ) ), esc_html__( 'Welcome Page', 'wpparallax' ), esc_html__( 'to get started with WPparallax.', 'wpparallax' ) );
			?>
			</p>
			<p><a class="button button-primary" href="<?php echo esc_url(admin_url( 'themes.php?page=welcome-page' )) ?>"><?php esc_html_e( 'Lets Get Started', 'wpparallax' ); ?></a>
				<span class="dismiss"><a href="?theme-info-ignore-notice"><?php esc_html_e('Dismiss','wpparallax') ?></a></span>
			</p>
			...
			public function welcome_styles_and_scripts() {
				wp_enqueue_style( 'wpparallax' . '-welcome-screen', get_template_directory_uri() . '/inc/welcome/css/welcome.css' );
				wp_enqueue_script( 'wpparallax' . '-welcome-screen', get_template_directory_uri() . '/inc/welcome/js/welcome.js', array( 'jquery' ) );

				wp_localize_script( 'wpparallax' . '-welcome-screen', 'operationWelcomeObject', array(
			...
			public function plugin_installer_callback(){
				...
				if ( ! current_user_can('install_plugins') ) {
					wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'wpparallax' ) );
				}
				...
				if (! wp_verify_nonce( $nonce, 'plugin_installer_nonce' )) {
					wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'wpparallax') );
				}
			...
			public function plugin_activation_callback() {
				...
				if(file_exists($plugin_file)) {
					...
					echo esc_html__('Plugin Does not Exists' , 'wpparallax');
				}
			...
			public function all_required_plugins_installed() {
		  	...
		  	public function get_local_dir_path($plugin) {
	      		...
	      		$url = wp_nonce_url(admin_url('themes.php?page=' . 'wpparallax' . '-welcome&section=actions_required'),'remote-file-installation');
	      	...
	      	public function admin_sidebar_contents(){ ?>
	      		<div class="operation-admin-side-wrapper">
					<div class="operation-side-box-wrapper operation-welcome-box-white">
						<div class="box-header op-box-header"><?php esc_html_e('Theme Documentation','wpparallax'); ?></div>
						<div class="box-content">
							<p><?php esc_html_e('We have online documentation to make your configuration process easy, go through the documentation to get more familiar with theme','wpparallax'); ?></p>
							<a href="<?php echo esc_url($this->strings['doc_link']);?>" class="button button-primary" target="_blank"><?php esc_html_e('Read Now','wpparallax'); ?></a>
						</div>
					</div>

					<div class="operation-side-box-wrapper operation-welcome-box-white">
						<div class="box-header op-box-header"><?php printf(esc_html__('Loving %1$s ? ','wpparallax'),$this->theme_name); ?></div>
						<div class="box-content">
							<p><?php esc_html_e('If you are enjoying theme,spread your love and rate our theme.','wpparallax'); ?></p>
							<a href="https://wordpress.org/support/theme/wpparallax/reviews/#new-post" class="button button-primary" target="_blank"><?php esc_html_e('Rate Now','wpparallax'); ?></a>
						</div>
					</div>
